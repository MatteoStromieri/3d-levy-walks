...
int main(int argc, char *argv[]) {
    ...
    // Define ranges
    double rangemu_LevyDistrib[] = {1.0, 1.2, 1.4, 1.6, 1.8, 2.0, 2.2, 2.4, 2.6, 2.8, 3.0};
    ...
    const char* list_shapes[] = {"Ball", "Line", "Disk"};
    int len_list_shapes = sizeof(list_shapes) / sizeof(list_shapes[0]);

    double range_probability[] = {0.1, 0.25, 0.5};
    ...
    long total_inner_iterations = (long)len_range_side * len_list_shapes * len_range_diam *
                                   len_rangemu_LevyDistrib * len_range_probability;
        ...
        for (int i_side = 0; i_side < len_range_side; ++i_side) {
            ...
            for (int i_shape = 0; i_shape < len_list_shapes; ++i_shape) {
                ...
                for (int i_D = 0; i_D < len_range_diam; ++i_D) {
                    ...
                    for (int i_ntargets = 0; i_ntargets < 1; ++i_ntargets) {
                        int n_targets = 1;
                        for (int i_probability = 0; i_probability < len_range_probability; ++i_probability) {
                            int n_walkers = 1;
                            double probability = range_probability[i_probability];
                            for (int i_mu = 0; i_mu < len_rangemu_LevyDistrib; ++i_mu) {
                                double mu = rangemu_LevyDistrib[i_mu];
                                for (int i = 0; i < 1; ++i) { // Only one distance for fixed target
                                    double detection_time = LevySearch3D_MultiWalker(n_walkers, "nest", n_volume, mu, lmax_current,
                                                                                    D, TargetShape, n_targets, fixed_target_dist, probability);

                                    fprintf(output_file, "%d,%.0f,%.1f,%d,%d,%s,%d,%.1f,%.2f,%.2f\n",
                                            n_walkers, n_volume, mu, lmax_current, D, TargetShape, n_targets, fixed_target_dist, detection_time, probability);
